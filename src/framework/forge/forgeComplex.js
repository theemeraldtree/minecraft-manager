...
const ForgeComplex = {
    ...
    calculateMavenPath: function(mvn) {
        this.log(`Calculating maven path for ${mvn}`)
        let split = mvn.split(':');
        let EXTENSION = '.jar';
        let isSpecial = false;
        if(mvn.indexOf('@') !== -1) {
            let splitAt = mvn.split('@');
    ...
    setupForge: function(profile, callback) {
    ...
    checkLibraryDone: function(profile) {
        let { totalLibraryCount, INSTALL_PROFILE, DATA, TEMP_PATH } = this.installState[profile.id];
        if(totalLibraryCount >= INSTALL_PROFILE.libraries.length) {
            this.log('Libraries are done downloading');
            this.log('Parsing data');
            for(let datum_name of Object.keys(INSTALL_PROFILE.data)) {
                this.log(`Reading datum ${datum_name}`);
                let datum = INSTALL_PROFILE.data[datum_name];
                let p = datum.client;
                if(p.substring(0, 1) === '[') {
                    ...
                    let outPath = path.join(`${TEMP_PATH}/data${p}`);
    ...
    checkDataDone: function(profile) {
        let { INSTALL_PROFILE, DATA, TEMP_PATH, totalDataCount } = this.installState[profile.id];
        if(totalDataCount >= Object.keys(INSTALL_PROFILE.data).length) {
            ...
            HTTPRequest.httpGet(
                ...
                let verURL = versions.versions.find(ver => ver.id === profile.version.minecraft.version).url;
                HTTPRequest.httpGet(
                    verURL
                ).then(verData => {
    ...
    executeNextProcessor: function(profile) {

        const getMainClass = (jar) => {
            const z = AdmZip(jar);
            let meta = z.readAsText('META-INF/MANIFEST.MF');
            for(let line of meta.split('\n')) {
                if(line.indexOf('Main-Class:') !== -1) {
                    return line.split(':')[1].replace(/\r?\n|\r/g, '').trim();
                }
            }
        }

        let { INSTALL_PROFILE, LIBRARIES, DATA, processorNumber, TEMP_PATH, callback } = this.installState[profile.id];
        if(processorNumber < INSTALL_PROFILE.processors.length) {
            let processor = INSTALL_PROFILE.processors[processorNumber];''
            let jar = LIBRARIES[processor.jar];
            let classpath = processor.classpath.map(library => LIBRARIES[library]);
            ...
            fs.copyFileSync(
                ...
                path.join(
                    libraryPath,
                    `/forge/mcm-${profile.id}-forge.jar`
                )
            );
            ...
            mkdirp(mcfpath, () => {
                ...
                let clientpath = path.join(LibrariesManager.getLibrariesPath(), `/net/minecraft/client`);
                mkdirp(clientpath, () => {
                    Global.copyDirSync(
                        path.join(
                            TEMP_PATH,
                            `/data/net/minecraft/client`
                        ),
                        path.join(
                            LibrariesManager.getLibrariesPath(),
