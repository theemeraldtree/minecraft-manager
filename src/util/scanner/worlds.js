...
const Worlds = {
    ...
    if (fs.lstatSync(dirPath).isDirectory()) {
      ...
      if (fs.existsSync(levelDatPath)) {
        ...
        nbt.parse(rawLevelDat, (err, leveldat) => {
           ...
           const worldID = `${Global.createID(fileName)}-${Math.floor(
  ...
  importWorldDir(profile, dirPath, doScan, forceName) {
    ...
    fs.readdirSync(dirPath).forEach(file => {
      ...
      if (doScan) {
        await Promise.all(worlds.map(world => new Promise(async res => {
          profile.addSubAsset('world', await this.scanWorld(
            profile,
            path.join(
              profile.gameDir,
              `/saves/${world.name}/`
